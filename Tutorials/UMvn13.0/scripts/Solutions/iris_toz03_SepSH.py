...
qplt.contourf(ukca, clevs, extend='both')
plt.gca().coastlines()
ax.gridlines()
...
proj = ccrs.Orthographic(central_longitude=0.0, central_latitude=ilat)
ax = plt.axes(projection=proj)
qplt.contourf(obs, clevs, extend='both')
plt.gca().coastlines()
ax.gridlines()
...
# levels for plotting
clevs=np.array([-25,-20,-15,-10,-5,-1,1,5,10,15,20,25])
# set the projection to look over the north pole
proj = ccrs.Orthographic(central_longitude=0.0, central_latitude=ilat)
ax = plt.axes(projection=proj)
cplot=qplt.contourf(diff_pc, clevs, cmap=cm.bwr, extend='both')
plt.gca().coastlines()
ax.gridlines()
...
proj = ccrs.Orthographic(central_longitude=0.0, central_latitude=ilat)
ax = plt.axes(projection=proj)
cplot=qplt.contourf(diff_abs, clevs, cmap=cm.bwr, extend='both')
plt.gca().coastlines()
ax.gridlines()
